...
export function DataTable<T extends Record<string, any>>({
  ...
  return (
    <div className="space-y-4">
      {/* Table Toolbar */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-surface-1 p-4 rounded-xl border border-[var(--color-border)]">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-xs bg-surface-2 border border-[var(--color-border)] rounded-lg text-text-primary placeholder:text-text-muted focus:outline-none focus:border-[var(--color-accent)]"
          />
        </div>
      ...
      <div className="bg-surface-1 rounded-xl border border-[var(--color-border)] overflow-hidden shadow-sm">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-text-muted">
                ...
                paginatedData.map((row, rowIdx) => (
                  <tr key={row._id || row.id || rowIdx} className="hover:bg-surface-2/60 transition-colors">
                    {columns.map((col, colIdx) => (
                      <td key={colIdx} className={`px-6 py-4 text-text-secondary ${col.className || ""}`}>
        ...
        {/* Pagination Footer */}
        <div className="px-6 py-3 border-t border-[var(--color-border)] bg-surface-2 flex items-center justify-between text-xs text-text-muted">
          <span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-1.5 rounded border border-[var(--color-border)] bg-surface-1 hover:bg-surface-3 disabled:opacity-40 transition-colors"
            >
              <ChevronLeft size={14} />
            </button>
