...
export function ServiceHero({
  ...
  return (
    <div className="relative pt-28 pb-12 md:pt-32 md:pb-16 overflow-hidden border-b border-[var(--color-border)]">
      ...
      <div className="container-site relative z-10">
        ...
        <nav aria-label="Breadcrumb" className="mb-6 md:mb-8">
          <ol className="flex items-center gap-2 text-xs sm:text-sm text-text-muted flex-wrap">
            ...
            <li className="text-[var(--color-accent)] font-medium truncate max-w-[200px] sm:max-w-none" aria-current="page">
              {title}
            </li>
          </ol>
        </nav>
        ...
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center">
          ...
          <div className="mb-4 md:mb-6 inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full border border-[var(--color-border-accent)] bg-surface-1/80 backdrop-blur-md shadow-xs">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[var(--color-accent)] opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-[var(--color-accent)]"></span>
            </span>
            <span className="text-xs uppercase tracking-widest text-text-secondary font-mono font-medium">
              {heroBadge || "ENGINEERING & DEVELOPMENT"}
            </span>
          </div>

          {/* Main Display Headline with Italic Champagne Accent */}
          <h1 className="font-display text-[clamp(2.75rem,6vw,6rem)] text-text-primary tracking-tight leading-[1.04] mb-4 md:mb-6">
            {mainTitle ? (
              <>
                {mainTitle}{" "}
          ...
          {tagline && (
            <p className="text-lg md:text-xl font-medium text-[var(--color-accent-dark)] mb-3 md:mb-4 max-w-2xl">
              {tagline}
            </p>
          )}

          {/* Short description */}
          <p className="text-base sm:text-lg text-text-secondary max-w-2xl mx-auto mb-6 md:mb-8 font-light leading-relaxed">
            {shortDescription}
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 mb-8 md:mb-10">
            <Button
              size="lg"
              href="/contact"
              className="rounded-full px-8 sm:px-9 bg-text-primary text-surface-1 hover:bg-text-secondary font-semibold shadow-xl"
            >
            ...
            <Button
              ...
              variant="outline"
              href="/portfolio"
              className="rounded-full px-7 sm:px-8 border-[var(--color-border)] hover:bg-surface-2"
            >
          ...
          <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 md:pt-8 border-t border-[var(--color-border)] text-left">
            <div className="p-4 rounded-xl bg-surface-1 border border-[var(--color-border)] shadow-xs">
              <div className="flex items-center gap-2 text-xs text-text-muted font-mono uppercase mb-1">
                <Zap size={14} className="text-[var(--color-accent)]" />
                <span>Performance</span>
              </div>
              <div className="text-xl sm:text-2xl font-bold font-display text-text-primary">
                &lt; 450ms TTFB
              </div>
